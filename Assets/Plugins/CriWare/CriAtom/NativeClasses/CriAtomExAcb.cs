...
public class CriAtomExAcb : IDisposable
{
	...
	/**
	 * <summary>ロード進捗状況の取得</summary>
	 * \par 説明:
	 * WebGLプラットフォームでACBに含まれているAACデータのロード状況を取得します。
	 * 0.0f～1.0fの値が返ります。1.0fが返ればロード完了です。
	 */
...
public class CriAtomExAcbAsync : IDisposable
{
	public enum Status
	{
		Loading,
	...
	private CriAtomExAcbAsync(IntPtr handle)
	{
		this.handle = handle;
	}
	...
	public void Dispose()
	{
		criAtomExAcbAsync_Release(this.handle);
	...
	{
		return criAtomExAcbAsync_GetStatus(this.handle);
	}

	public CriAtomExAcb MoveAcb()
	{
		IntPtr acbHandle = criAtomExAcbAsync_MoveAcbHn(this.handle);
		return (acbHandle != IntPtr.Zero) ? new CriAtomExAcb(acbHandle) : null;
	}

	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern IntPtr criAtomExAcbAsync_LoadAcbFile(
		IntPtr acb_binder, string acb_path, IntPtr awb_binder, string awb_path);
	
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern IntPtr criAtomExAcbAsync_LoadAcbData(
		byte[] acb_data, int acb_size, IntPtr awb_binder, string awb_path);
	
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern void criAtomExAcbAsync_Release(IntPtr acbasync);
	
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern Status criAtomExAcbAsync_GetStatus(IntPtr acbasync);
	
	[DllImport(CriWare.pluginName, CallingConvention = CriWare.pluginCallingConvention)]
	private static extern IntPtr criAtomExAcbAsync_MoveAcbHn(IntPtr acbasync);
}
#endif
